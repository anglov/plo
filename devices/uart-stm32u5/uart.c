...
enum { cr1 = 0, cr2, cr3, brr, gtpr, rtor, rqr, isr, icr, rdr, tdr, presc, autocr,  };


static int uartLut[] = { PLO_UART1, PLO_UART2, PLO_UART3, PLO_UART4, PLO_UART5 };
...
static const struct {
	...
	{ (void *)UART1_BASE, UART1_CLK, UART1_IRQ, UART1_RX, UART1_TX },
	{ (void *)UART2_BASE, UART2_CLK, UART2_IRQ, UART2_RX, UART2_TX },
	{ (void *)UART3_BASE, UART3_CLK, UART3_IRQ, UART3_RX, UART3_TX },
	{ (void *)UART4_BASE, UART4_CLK, UART4_IRQ, UART4_RX, UART4_TX },
	{ (void *)UART5_BASE, UART5_CLK, UART5_IRQ, UART5_RX, UART5_TX }
};
...
static int uart_done(unsigned int minor)
{
	...
	*(uart->base + cr1) = 0;
	...
	hal_cpuDataMemoryBarrier();
	_stm32_gpioConfig(uartInfo[minor].rx.port, uartInfo[minor].rx.pin, 0, 3, 0, 0, 0);
	_stm32_gpioConfig(uartInfo[minor].tx.port, uartInfo[minor].tx.pin, 0, 3, 0, 0, 0);
...
static int uart_init(unsigned int minor)
{
	...
	_stm32_gpioConfig(uartInfo[minor].rx.port, uartInfo[minor].rx.pin, 2, uartInfo[minor].rx.af, 0, 0, 0);
	_stm32_gpioConfig(uartInfo[minor].tx.port, uartInfo[minor].tx.pin, 2, uartInfo[minor].tx.af, 0, 0, 0);
	hal_cpuDataMemoryBarrier();
